...
void Configuration::DrawMainCamera()
{
	if (ImGui::CollapsingHeader("Main camera", ImGuiTreeNodeFlags_DefaultOpen))
	{
		...
		if (ImGui::Checkbox("Perspective (activated) - Orthogonal (deactivated)", &App->scene_base->main_camera->perspective) ||
			ImGui::DragFloat("Fov Y",  &App->scene_base->main_camera->fov_y,	0.25f, 0.0f, 180.0f) ||
			ImGui::DragFloat("Z Near", &App->scene_base->main_camera->z_near,	0.25f, 1.0f, App->scene_base->main_camera->z_far) ||
